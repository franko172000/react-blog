...
class PostControllerTest extends BaseTestCase
{
    ...
    private string $baseUserPath = "spa/user/";
    ...
    public function testUserCanAddPost()
    {
        ...
        $response = $this->actingAs($user)
            ...
            ->post($this->baseUserPath  . 'add-post', $postData, [
    ...
    public function testUserCanGetPostDetail()
    {
        $user = $this->makeUserPost();
        $post = $user->posts()->first();

        $response = $this->get('spa/post/' . $post->id, [
                'Accept' => 'application/json'
            ]);
        ...
        $responseData = $data['data'];
        $this->assertEquals($post->id, $responseData['id']);
        $this->assertEquals($post->title, $responseData['title']);
        $this->assertEquals($post->description, $responseData['description']);
        $this->assertEquals($post->publication_date->format('F j, Y'), $responseData['publicationDate']);
        $response->assertStatus(200);
    }

    public function test404IfPostDoesNotExists()
    {
        $user = $this->makeUserPost();
        $post = $user->posts()->first();

        $response = $this->get('spa/post/3000', [
            'Accept' => 'application/json'
        ]);

        $this->handle404Assertions($response);
    }
    ...
    public function testGuestUserCanNotAddPost()
    {
        ...
        $response = $this->post($this->baseUserPath  . 'add-post', $postData, [
            'Accept' => 'application/json'
        ]);
    ...
    public function testUserCanGetPosts()
    {
        $totalPost = 50;
        $defaultLimit = 20;
        $user = $this->makeUserPost($totalPost);

        $response = $this->actingAs($user)
            ->withSession(['USER_SESSION' => time()])
            ->get($this->baseUserPath  . 'posts', [
                'Accept' => 'application/json'
            ]);
    ...
    public function testUserCanGetPostsWithCustomLimit()
    {
        ...
        $response = $this->actingAs($user)
            ->withSession(['USER_SESSION' => time()])
            ->get($this->baseUserPath  . 'posts?limit=' . $limit, [
                'Accept' => 'application/json'
            ]);
    ...
    public function testPostsPagination()
    {
        ...
        $response = $this->actingAs($user)
            ->withSession(['USER_SESSION' => time()])
            ->get($this->baseUserPath  . 'posts?limit=' . $limit . '&page=2', [
                'Accept' => 'application/json'
            ]);
    ...
    private function makeUserPost(int $count = 1)
    {
        $user = $this->makeUser();
        $user->posts()
            ->saveMany(Post::factory()
